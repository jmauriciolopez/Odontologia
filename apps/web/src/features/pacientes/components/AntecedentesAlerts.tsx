...
export const AntecedentesAlerts: React.FC<AntecedentesAlertsProps> = ({ antecedentes = [], onAdd }) => {
  ...
  return (
    <div className="medical-card p-6 bg-[var(--card-bg)] border-[var(--sb-border)] shadow-lg shadow-slate-200/50 dark:shadow-none">
      <div className="flex justify-between items-center mb-6">
        <div className="flex items-center gap-2">
           ...
           <div>
              <h3 className="font-bold text-[var(--sb-text)] leading-none">Alertas Médicas</h3>
              <p className="text-[10px] font-bold text-[var(--sb-text-muted)] uppercase tracking-widest mt-1">Antecedentes Críticos</p>
           </div>
        </div>
        <button
          onClick={onAdd}
          className="p-2 hover:opacity-800 rounded-xl text-[var(--sb-text-muted)] hover:text-primary transition-all active:scale-90"
      ...
      <div className="grid gap-3">
        {antecedentes.length > 0 ? (
                ...
                <motion.div
                  ...
                  className={cn(
                    ...
                    isCritical
                      ...
                      : "bg-[var(--sb-active-bg)]/50 border-[var(--sb-border)] text-[var(--sb-text-muted)]"
                  )}
                >
          ...
          <div className="flex flex-col items-center justify-center py-8 px-4 text-center gap-3 bg-[var(--sb-active-bg)] rounded-3xl border border-dashed border-[var(--sb-border)]">
             <ShieldCheck size={24} className="text-emerald-500 opacity-50" />
             <p className="text-xs font-bold text-[var(--sb-text-muted)] italic">Sin antecedentes médicos registrados</p>
          </div>
        )}
      </div>
    </div>
