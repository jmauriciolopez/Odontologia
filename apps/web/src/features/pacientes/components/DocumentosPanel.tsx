    ...
    <div className="flex flex-col gap-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div className="flex flex-col gap-2">
            ...
            <label className="flex items-center justify-center gap-2 bg-slate-900 hover:bg-slate-800 text-white p-3 rounded-2xl font-bold shadow-lg shadow-slate-900/20 transition-all active:scale-95 cursor-pointer text-xs">
                <Plus size={16} />
                Subir Radiografía
      ...
      {/* 2. Search & Filter Bar */}
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4 bg-white dark:bg-slate-900 p-2 rounded-[2rem] border border-slate-100 dark:border-slate-800 shadow-sm">
        <div className="flex p-1 bg-slate-50 dark:bg-slate-800/50 rounded-2xl w-full sm:w-auto">
      ...
      {filteredFiles.length > 0 ? (
          ...
          <AnimatePresence mode="popLayout">
            {filteredFiles.map((file) => (
              <motion.div
                ...
                className="group relative medical-card p-4 flex flex-col gap-4 bg-white/50 hover:bg-white transition-all hover:shadow-xl hover:shadow-blue-500/5 border-slate-100 dark:border-slate-800"
              >
                <div className="aspect-[4/3] rounded-2xl bg-slate-50 dark:bg-slate-800/50 flex items-center justify-center relative overflow-hidden group-hover:bg-blue-50/50 transition-all">
                ...
                <div className="space-y-1">
                    <h4 className="font-bold text-xs text-slate-800 dark:text-slate-200 truncate pr-4" title={file.nombreArchivo}>
                        {file.nombreArchivo}
                    </h4>
                    <div className="flex items-center justify-between">
                        ...
                        <span className={cn(
                            ...
                            file.type === 'radiografia' ? "border-amber-100 text-amber-500 bg-amber-50" : "border-slate-100 text-slate-500 bg-slate-50"
                        )}>
                            {file.type}
                        </span>
                    </div>
        ...
        <div className="flex flex-col items-center justify-center py-32 px-6 text-center gap-4 bg-slate-50/50 dark:bg-slate-800/10 rounded-[4rem] border-2 border-dashed border-slate-200 dark:border-slate-800">
             <div className="h-20 w-20 bg-white dark:bg-slate-900 rounded-3xl flex items-center justify-center text-3xl shadow-sm border border-slate-100 dark:border-slate-800 rotate-12">📁</div>
             <div className="space-y-1">
               <p className="font-bold text-lg text-slate-800 dark:text-slate-200 tracking-tight">Su Bóveda está Vacía</p>
               <p className="text-xs text-slate-400 max-w-[280px] font-medium leading-relaxed">
...
const PremiumStatCard = ({ title, value, icon, color }: { title: string, value: number, icon: React.ReactNode, color: 'blue' | 'slate' | 'amber' }) => {
    const colors = {
        ...
        slate: "bg-white dark:bg-slate-900 text-slate-900 dark:text-white border border-slate-100 dark:border-slate-800 shadow-slate-200/50",
    ...
    return (
        <div className={cn("p-6 rounded-[2rem] flex items-center justify-between group transition-all hover:scale-[1.02]", colors[color])}>
            <div className="space-y-1">
                <p className={cn("text-[10px] font-black uppercase tracking-widest", color === 'slate' ? "text-slate-400" : "opacity-80")}>
                    {title}
                </p>
            ...
            <div className={cn("h-12 w-12 rounded-2xl flex items-center justify-center transition-transform group-hover:rotate-12", color === 'slate' ? "bg-slate-50 dark:bg-slate-800 text-slate-400" : "bg-white/20 text-white")}>
                {icon}
            </div>
        </div>
    );
